...
def upgrade():
    ...
    def addcol(table, name, coldef):
        ...
        if name not in cols:
            ...
            op.add_column(table, sa.Column(name, col, **extra))

    # Restaurant configuration
    for n, c in {
        "info": sa.Text(),
        "accept_orders": (sa.Boolean(), {"server_default": sa.true(), "nullable": False}),
        "pause_message": sa.String(300),
        "delivery_enabled": (sa.Boolean(), {"server_default": sa.true(), "nullable": False}),
        "pickup_enabled": (sa.Boolean(), {"server_default": sa.true(), "nullable": False}),
        "delivery_fee": sa.Numeric(12, 2),
        "minimum_order": sa.Numeric(12, 2),
        "delivery_zones": sa.JSON(),
        "prep_min": sa.Integer(),
        "prep_max": sa.Integer(),
        "theme_color": sa.String(20),
    }.items():
        addcol("restaurant", n, c)

    # Product configuration
    for n, c in {
        "previous_price": sa.Numeric(12, 2),
        "stock_control": (sa.Boolean(), {"server_default": sa.false(), "nullable": False}),
        "display_order": (sa.Integer(), {"server_default": "0", "nullable": False}),
        "label": sa.String(40),
        "nutrition": sa.Text(),
        "prep_min": sa.Integer(),
        "prep_max": sa.Integer(),
        "is_combo": (sa.Boolean(), {"server_default": sa.false(), "nullable": False}),
    }.items():
        addcol("restaurant_product", n, c)

    # Order and snapshot fields
    for n, c in {
        "delivery_zone": sa.String(160),
        "subtotal": sa.Numeric(12, 2),
        "discount": sa.Numeric(12, 2),
        "delivery_fee": sa.Numeric(12, 2),
        "scheduled_for": sa.DateTime(),
    }.items():
        addcol("order", n, c)

    for n, c in {"modifiers_json": sa.Text(), "item_note": sa.Text()}.items():
        addcol("order_item", n, c)

    # Defaults seguras para filas existentes
    ...
    if "restaurant_product" in tables:
